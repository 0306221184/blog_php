...
<table class="table-primary tableManagement-table">
    ...
    <tbody class="tableManagement-table__tbody">
        ...
        <?php
            ...
            ?>
        <?php if ($userRole == "ADMIN" && $userId == Session::get("userId")): ?>
        ...
        <?php
                        $nameManageBtn = $item['isActive'] == false ? "Approving" : "Excepted";
                        $hrefManageBtn = $userId != Session::get('userId')
                            ? "./post.php?postId=" . $item['id'] . "&authorId=" . $userId
                            : "./editPost.php?postId=" . $item['id'];
                        ?>
        ...
        <tr class="table-tbody__tr">
            ...
            <td scope="col"><?= Format::textShorten($item["title"]) ?></td>
            <td scope="col"><?= Format::textShorten($item["category"]) ?></td>
            <td scope="col"><a href="<?= $hrefManageBtn ?>"><button
                        class="myBtn"><?= $userId != Session::get('userId') ? "Watch the post" : "Edit" ?></button></a>
            </td>
            <?php if ($userId == Session::get('userId')): ?>
            <td scope="col"><a href="./src/features/adminDeletePost.php?postId=<?= $item['id'] ?>"><button
                        class="myBtn">Delete</button></a></td>
        ...
        <p class="text-center fs-3 text-success">You haven't any post yet <a href="./writePost.php">write now!!</a></p>
        <?php endif; ?>
        <?php endif; ?>
