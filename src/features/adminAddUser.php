...
if ($userRole == "ADMIN") {
            ...
            if (isset($_FILES['avatarUpload']) && $_FILES['avatarUpload']['error'] == 0) {
                ...
                if (in_array($fileExtension, $allowedExtensions)) {
                    ...
                    if ($avatarFileSize <= $maxFileSize) {
                        ...
                        header("Location: ../../user.php?path=add-user&userId=$userId&error=Avatar image have to less than 5MB!!");
                        die();
                    }
                } else {
                    header("Location: ../../user.php?path=add-user&userId=$userId&error=Avatar image have to a image!!");
                    die();
                }
            } else {
                header("Location: ../../user.php?path=add-user&userId=$userId&error=Avatar is required!!");
                die();
            }
        } else {
    ...
    header("Location: ../../user.php?path=add-user&userId=$userId&error=Permission denied!!");
    die();
}